...
def call_automata(state,free_ahead,lettuce_ahead,lettuce_here,water_ahead,water_here,hungry,thirsty,full_food,full_drink):
    if state==0:
        if hungry and not lettuce_here:
            return 1,'forward'
        elif not full_food and lettuce_here:
            return 0,'eat'
        elif thirsty and not water_here:
            return 2,'forward'
        elif not full_drink and water_here:
            return 0,'drink'
        else:
            return 0,'wait'
    if state==1:
        if free_ahead and not lettuce_here:
            return 1,'forward'
        elif lettuce_here:
            return 0,'eat'
        else:
            return 1,'right'
    if state==2:
        if free_ahead and not water_here:
            return 2,'forward'
        elif water_here:
            return 0,'drink'
        else:
